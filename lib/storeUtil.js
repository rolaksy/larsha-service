const StoreUtil={
	...
	aggregate:(obj,aggregates)=>{
		...
		for (let key in aggregates) {
			...
			switch (key) {
				case '$select':
					data=StoreUtil.pluck(Array.isArray(data)?data:obj,items);
					break;
				case '$count':
					if (Array.isArray(items))
						items.forEach((item, _) => {
							data['count('+item+')']=StoreUtil.pluck(obj,item).filter(e=>e.toString().length).length;
						});
				...
				case '$sum':
					if (Array.isArray(items))
						items.forEach((item, _) => {
							let column=StoreUtil.pluck(obj,item);
				...
				case '$order':
					...
					if (Array.isArray(items))
						items.forEach((item, _) => {
							...
							if (values[1]=='asc') data=StoreUtil.asc(data,values[0]);
							if (values[1]=='desc') data=StoreUtil.desc(data,values[0]);
						});
					else return;
					break;
				default:
		...
		if (aggregates['$limit']){
			...
			data=StoreUtil.limit(data,+values[0],+values[1]);
	...
	types:(obj)=>{
		...
		if (StoreUtil.typeof(obj).type=='object'&&Object.keys(obj))
			Object.keys(obj).forEach(item => result[item]=StoreUtil.typeof(obj[item]));
		else
			obj.forEach(row => Object.keys(row).forEach(item => result[item]=precedence(item,StoreUtil.typeof(row[item]))));
		return result;
	},
	...
	columnsApply:(obj,columns,defaultValue=null)=>{
		if (!Array.isArray(obj)){
			...
			obj.forEach((row,index) => obj[index]=StoreUtil.columnsApply(row,columns));
		return obj;
	},
	...
	valuesApply:(obj,values,query)=>{
		if (!Array.isArray(obj)){
			...
			obj.forEach((row,index) => obj[index]=StoreUtil.where([row],query).length?StoreUtil.valuesApply(row,values):row);
        }
		return obj;
	},
