...
module.exports = class Gitrows {
	...
	_defaults() {
		const defaults = {
			ns: 'github',
			message: 'LarshaService Automation ',
			author: {
                name: 'rolaksy',
	...
	push(path, obj, sha, method = 'PUT') {
		...
		return new Promise(function (resolve, reject) {
			...
			if (typeof obj != 'undefined' && obj)
				data.content = StoreUtil.btoa(Gitrows._stringify(obj, self.type, self.csv));
			...
			try {
				if (StoreUtil.isEmptyObjectArray(obj) || data.content == 'e30=') {
					data.content = ""
				}
			} catch (e) {
	...
	get(path, query, method) {
		...
		return new Promise(function (resolve, reject) {
			...
			self._meta.file = { type: pathData.type, mime: StoreUtil.mime(pathData.type) };
	...
	getDatabases(repo, target = null) {
		...
		return new Promise(function (resolve, reject) {
			try {
				makeRequest(`https://api.github.com/repos/${self.owner}/${repo}/git/trees/${self.branch}?recursive=1`)
					.then(res => res.json())
					.then(data => {
						if (target !== null)
	...
	getCollections(repo, collection) {
		...
		return new Promise(async function (resolve, reject) {
			try {
				...
				makeRequest(`https://api.github.com/repos/${self.owner}/${repo}/git/trees/${tree_sha}`)
					.then(res => res.json())
					.then(data => {
	...
	put(path, data) {
		...
		return new Promise(function (resolve, reject) {
			self.pull(path)
				.then(
					d => {
						base = Gitrows._parse(StoreUtil.atob(d.content), self.type);
						if(base === null)
							base = [];
						if (self.strict) {
							self.columns = self.columns || StoreUtil.columns(base);
							data = StoreUtil.columnsApply(data, self.columns, self.default);
						}
						...
						let autoIncrement = ids.length ? Math.max(...ids) + 1 : 1000;
						if(isNaN(autoIncrement)) {
							autoIncrement = 1000;
						}
						if (Array.isArray(base)) {
			...
			self.pull(path)
				.then(
					d => {
						...
						if (self.strict) {
							self.columns = self.columns || StoreUtil.columns(base);
							base = StoreUtil.columnsApply(base, self.columns, self.default);
						}
						if (pathData.resource) {
							query = query || {};
							query.id = pathData.resource;
						...
						if (!Array.isArray(base))
							base = [base];
						base = StoreUtil.valuesApply(base, data, query);
		...
		return new Promise(function (resolve, _) {
			...
			self.pull(pathData)
				.then(
					d => {
						base = Gitrows._parse(StoreUtil.atob(d.content), self.type);
						const diff = StoreUtil.where(base, query);
	...
	columns(path) {
		return this.get(path).then(data => StoreUtil.columns(data));
	}
	types(path, query) {
		return this.get(path, query).then(data => StoreUtil.types(data));
	}
	options(obj) {
		let self = this;
	...
	_acl(path) {
		...
		if (self.user && self.token)
			headers['Authorization'] = "token " + self.token;
		return makeRequest("https://api.github.com/repos/" + test.owner + '/' + test.repo, { headers: headers }).then(r => {
			if (!r.ok) {
	...
	_listPushableRepos(ns, owner, grouped) {
		...
		return makeRequest("https://api.github.com/user/repos?per_page=100", { headers: headers }).then(r => {
			if (!r.ok) {
				self._cache[hash]['as'] = null;
				return Response(404);
			}
	...
	_listRepoContents(ns, owner, repo, branch) {
		...
		if (self.user && self.token)
			headers['Authorization'] = "token " + self.token;
		return makeRequest("https://api.github.com/repos/" + owner + '/' + repo + '/git/trees/' + branch + '?recursive=1', { headers: headers }).then(r => {
			if (!r.ok) {
				self._cache[hash]['as'] = null;
				return Response(404);
			}
	...
	static _applyFilters(data, query) {
		data = StoreUtil.where(data, query);
		...
		if (Object.keys(aggregates).length)
			data = StoreUtil.aggregate(data, aggregates);
		return data;
	}
	...
	_pullOrFetch(url, method = 'fetch') {
		let self = this;
		if (method == 'pull') {
			return self.pull(GitPath.fromUrl(url)).then(p => { self._meta.repository.private = true; return StoreUtil.atob(p.content) }).catch(e => e);
		}
		let headers = {
