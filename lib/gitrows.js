...
module.exports = class Gitrows {
	...
	_defaults() {
		const defaults = {
			...
			author: { name: "Laks Yalamati", email: "9laksy@gmail.com" },
			csv: { delimiter: "," },
			strict: false,
			default: null
		};
	...
	pull(path) {
		let self = this;
		console.log('pull token: ', self.token);
	...
	_pullOrFetch(url, method = 'fetch') {
		console.log('_pullOrFetch url:', url);
		let self = this;
		if (method == 'pull') {
		...
		console.log('_pullOrFetch headers:', headers);
